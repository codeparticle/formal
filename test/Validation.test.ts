...
describe('Validation', () => {
	...
	it('Takes a list of validators made with createValidator and checks a value accordingly', () => {
		const testValue = Validator.of<typeof testObject>(
			isObject,
	...
	it('Collects errors when multiple conditions fail', () => {
		const failedObject = Validator.of(isObject, getProp('nested', 'property', 'fail')).validate(
			testObject,
		)
		...
		expect(failedNumber.then(testOpts)).toMatchObject(['Value wow is not a number'])
	})
})

describe('Rule', () => {
	const customRule = createRule({
		condition: (val: number) => val > 3,
	...
	it('allows for overwriting the message of an existing rule using withMessage', () => {
		...
		const overwriteFn = withMessage<number>((d) => `Man, that's a crooked ${d}`)

		expect(newRule.check(2).isSuccess).toBe(false)
		expect(overwriteFn(newRule).check(2).errors[0]).toBe(`Man, that's a crooked 2`)
...
describe('validateObject', () => {
	it('can validate over an object, returning the original with an attached error object containing messages', () => {
		...
		const daveIsSpelledBackwardsSomewhere = createRule({
			condition: (arr: { name: string }[]) => arr.some((o) => o.name === 'evad'),
		...
		const rules = {
			...
			confirmationEmail: [(values) => isEqualTo(values['email']), isNonEmptyString, isValidEmail],
			// non-required fields
			city: [isString],
			names: [hasExactlyTwoNameObjects, daveIsSpelledBackwardsSomewhere],
		}

		const validate = validateObject(rules)
